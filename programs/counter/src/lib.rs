...
use whitelist::{self};
declare_id!("41wqstn9QMyDvPDNvndusXmkhXy9Qx9kfjnLeEf4dDhj");
...
pub mod counter {
    ...
    pub fn update(ctx: Context<UpdateCounter>) -> Result<()> {
        let counter = &mut ctx.accounts.counter;
        assert!(counter.initialized == true);
        let wallet_address = &ctx.accounts.wallet_address;
        let count = counter.count;
        let admin = ctx.accounts.authority.key();
        let seeds= ["counter".as_bytes().as_ref(), admin.as_ref(), &[counter.bump]];
        ...
        match whitelist::cpi::check_address(check_ctx, wallet_address.key()){
