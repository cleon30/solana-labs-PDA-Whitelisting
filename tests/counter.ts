...
import { waitForDebugger } from "inspector";
  ...
  describe("counter",() => {
    ...
    before(async () => {
        ...
        await CounterProgram.provider.connection.confirmTransaction(
            await CounterProgram.provider.connection.requestAirdrop(
              authority.publicKey,
              airdrop_number*LAMPORTS_PER_SOL
            ),
            "confirmed"
        );       
    });
    ...
    //   assert.equal(counter.count.toNumber(), 3);
            }
            await new Promise(f => setTimeout(f, cleon_theory_time *1000));
            let counter = await CounterProgram.account.counter.fetch(counterPDA);
